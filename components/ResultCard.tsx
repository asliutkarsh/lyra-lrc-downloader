...
interface ResultCardProps {
  // CHANGED: Renamed from 'match' to 'matchData' to match SingleSongSearch
  matchData: MatchResult;
  filenameFormat?: FilenameFormat;
}

const ResultCard: React.FC<ResultCardProps> = ({ matchData, filenameFormat }) => {
  // SAFETY CHECK: Prevent crash if data hasn't loaded yet
  if (!matchData || !matchData.track) {
    return null;
  }

  // CHANGED: Destructure from matchData
  const { track, confidenceScore, confidenceReasons } = matchData;
          ...
          <div className="flex flex-wrap gap-2 w-full">
            {track.syncedLyrics && (
              <>
                <button
                  ...
                  className="px-4 py-2.5 rounded-lg text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 font-medium text-sm transition-colors whitespace-nowrap"
                >
                  {showPreview ? 'Hide Text' : 'View Text'}
                </button>
                <button
                  onClick={() => setShowKaraoke(true)}
                  className="px-4 py-2.5 rounded-lg text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-800 font-medium text-sm flex items-center justify-center gap-2 transition-colors whitespace-nowrap"
                >
            ...
            {track.syncedLyrics && (
              <button
                ...
                className="px-4 py-2.5 rounded-lg text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 font-medium text-sm flex items-center justify-center gap-2 transition-colors whitespace-nowrap"
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm shadow-md shadow-indigo-500/20 flex items-center justify-center gap-2 transition-all transform hover:-translate-y-0.5 whitespace-nowrap"
            >
              <Download size={16} /> Download LRC
            </button>
          </div>
        </div>
